...
use serde_ipld_dagcbor::{self, Serializer};
...
mod std_tests {
    ...
    #[test]
    fn test() {
        let serialized = serde_ipld_dagcbor::ser::to_vec_packed(&EXAMPLE).expect("bennofs 1");
        println!("{:?}", serialized);
        let deserialized: Example = serde_ipld_dagcbor::from_slice(&serialized).expect("bennofs 2");
