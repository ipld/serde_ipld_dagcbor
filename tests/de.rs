...
fn test_map_duplicate_keys() {
    // `{"a": 0, "a": 1}`: a key must be strictly greater than the previous one, so equal
    // (duplicate) keys are rejected too. Decoding into a `BTreeMap`, which silently overwrites
    // duplicate keys instead of rejecting them (unlike the `Ipld` visitor), shows the rejection
    // comes from the ordering check itself rather than from the target type.
    ...
    #[cfg(feature = "less-strict-decoding")]
    assert_eq!(result.unwrap(), BTreeMap::from([("a".to_string(), 1)]));
}
